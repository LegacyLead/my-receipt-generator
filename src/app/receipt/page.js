      ...
      <div className="max-w-5xl mx-auto p-4 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4" style={{ animation: 'slideUp 0.4s cubic-bezier(0.16,1,0.3,1) 0.05s both' }}>
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm space-y-3">
            ...
            {/* Column headers */}
            <div className="grid grid-cols-[1fr_52px_80px_28px] gap-2 px-1">
            ...
            <div className="space-y-2">
              {items.map((item, idx) => (
                <div key={idx}
                  className="grid grid-cols-[1fr_52px_80px_28px] gap-2 items-center"
                  ...
                  <input type="text" placeholder="Item description" value={item.name}
                    ...
                    className="px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 focus:bg-white transition-all duration-200" />
                  <input type="number" placeholder="1" value={item.qty}
                    onChange={(e) => updateItem(idx, 'qty', parseInt(e.target.value) || 0)}
                    className="px-1 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-center text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 focus:bg-white transition-all duration-200" />
                  <div className="relative group">
                  ...
                  {items.length > 1 ? (
                    <button onClick={() => removeItem(idx)}
                      className="flex items-center justify-center w-7 h-7 rounded-lg text-slate-300 hover:text-red-500 hover:bg-red-50 active:scale-90 transition-all duration-150">
                      <Trash2 size={13} />
                    </button>
                  ) : <span />}
                </div>
        ...
        <div className="flex justify-center items-start" style={{ animation: 'slideUp 0.4s cubic-bezier(0.16,1,0.3,1) 0.1s both' }}>
          <div className="w-full max-w-[400px]">
            ...
            {/* ← This outer wrapper gives the shadow/frame without being captured */}
            <div className="rounded-2xl shadow-xl shadow-slate-900/10 border border-slate-200 overflow-hidden">
              {/* ← Only this inner div gets captured as the image */}
