...
import { Lock, Mail, Store, ArrowRight, Eye, EyeOff } from 'lucide-react';
          ...
          <div className="p-8">
            ...
            <div className="flex flex-col items-center mb-8 text-center">
              ...
              <p className="text-sm text-slate-500 mt-1.5">
                {isSignUp ? 'Set up your business and start issuing receipts' : 'Sign in to your receipt dashboard'}
              </p>
            </div>

            {/* Alert */}
            {message && (
              <div className={`flex items-start gap-2.5 p-3.5 rounded-2xl text-sm mb-6 border ${isError ? 'bg-red-50 text-red-700 border-red-100' : 'bg-emerald-50 text-emerald-700 border-emerald-100'}`}>
                <span>{isError ? '⚠' : '✓'}</span>
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              ...
              <div className="group">
                ...
                <div className="relative">
                  <Mail size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-500 transition-colors duration-200 pointer-events-none" />
                  <input
              ...
              <div className="group">
                ...
                <div className="relative">
                  <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-500 transition-colors duration-200 pointer-events-none" />
                  <input
                    type={showPass ? 'text' : 'password'} required placeholder="••••••••"
                    value={password} onChange={(e) => setPassword(e.target.value)}
                    className="w-full pl-10 pr-11 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm text-slate-900 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 focus:bg-white transition-all duration-200"
                  />
              ...
              <button type="submit" disabled={loading}
                className="group w-full flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-700 active:scale-[0.98] text-white font-semibold py-3.5 rounded-2xl transition-all duration-200 text-sm disabled:opacity-60 shadow-md shadow-slate-900/15 mt-2">
                {loading ? (
                  ...
                  <>
                    <span>{isSignUp ? 'Register Business' : 'Sign In'}</span>
